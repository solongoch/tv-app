...
export class SeasonsListServiceService {
  ...
  getShowSeasons(showId : number){
    return this.httpClient.get<ISeasonListData[]>(`${environment.baseurl}${environment.seasonEndpoint}${showId}/seasons`).pipe(map(( seasons:ISeasonListData[] )=>seasons.map(season => this.transformISeasonDataToView(season))));
  }
